...
Quaternion::Quaternion(const Rotator& rotator)
{
	...
	const auto cy = BEM::Cosine(rotator.Y * 0.5f);
	const auto sy = BEM::Sine(rotator.Y   * 0.5f);
	const auto cp = BEM::Cosine(rotator.X * 0.5f);
	const auto sp = BEM::Sine(rotator.X   * 0.5f);
	const auto cr = BEM::Cosine(rotator.Z * 0.5f);
	const auto sr = BEM::Sine(rotator.Z   * 0.5f);
...
Quaternion::Quaternion(const AxisAngle& axisAngle) :
	X(axisAngle.X * BEM::Sine(axisAngle.Angle / 2)),
	Y(axisAngle.Y * BEM::Sine(axisAngle.Angle / 2)),
	Z(axisAngle.Z * BEM::Sine(axisAngle.Angle / 2)),
	Q(BEM::Cosine(axisAngle.Angle / 2))
{
}
