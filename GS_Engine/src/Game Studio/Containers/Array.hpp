...
public:
	...
	{
		GS_ASSERT(size > Size, "Requested size for array resize is greater than Array's statically allocated size!")
		this->length = size;
	}
	...
	{
		GS_ASSERT(i > Size, "Out of Bounds! Requested index is greater than the Array's statically allocated size!")
	...
	{
		GS_ASSERT((this->length + 1) > Size, "Array is not long enough to insert any more elements!")
