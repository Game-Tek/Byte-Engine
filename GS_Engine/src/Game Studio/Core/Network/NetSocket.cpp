...
bool NetSocket::Send(const NetSocketSendInfo& NSSI_)
{
    ...
    int sent_bytes = sendto(handle, static_cast<const char*>(NSSI_.Data), NSSI_.Size, 0, reinterpret_cast<sockaddr*>(&addr), sizeof(sockaddr_in));
...
bool NetSocket::Receive(const NetSocketReceiveInfo& NSRI_)
{
    ...
    int bytes = recvfrom(handle, reinterpret_cast<char*>(NSRI_.Buffer), NSRI_.BufferSize, 0, reinterpret_cast<sockaddr*>(&from), &fromLength);
