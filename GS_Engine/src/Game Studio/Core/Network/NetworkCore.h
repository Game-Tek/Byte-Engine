...
struct IpEndpoint
{
	 ...
	 */
	[[nodiscard]] uint32 IntFromAddress() const { return (Address[0] << 24) | (Address[1] << 16) | (Address[2] << 8) | Address[3]; }
	/**
	 ...
	 */
	void AddressFromInt(const uint32 int_address_) { Address[0] = ((int_address_ >> 24) & 0xFF), Address[1] = ((int_address_ >> 16) & 0xFF), Address[2] = ((int_address_ >> 8) & 0xFF), Address[3] = (int_address_ & 0xFF); }
};
...
/**
 * \brief Basic structure for a network packet in GS engine.
 */
struct Packet
{
