...
void GBufferRenderPass::Render()
{
	...
	GBufferPassProgram.ViewMatrix.Set(RendererOwner->GetScene()->GetViewMatrix());
	GBufferPassProgram.ProjMatrix.Set(RendererOwner->GetScene()->GetProjectionMatrix());
	...
	{
		GBufferPassProgram.ModelMatrix.Set(GSM::Translation(RendererOwner->GetScene()->RenderProxyList[i]->GetOwner()->GetPosition()));
