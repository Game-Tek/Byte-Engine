...
public:
	...
	void Recreate(const Vk_Surface& _Surface, VkFormat _SurfaceFormat, VkColorSpaceKHR _SurfaceColorSpace, VkExtent2D _SurfaceExtent);
	uint32 AcquireNextImage(const Vk_Semaphore& _ImageAvailable);
	[[nodiscard]] FVector<VkImageView> GetImages() const;
