...
VulkanMesh::VulkanMesh(VkDevice* _Device, const VKCommandPool& _CP, void* _VertexData, size_t _VertexDataSize,
	...
	IndexBuffer(CreateVKBufferCreator(_Device, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
	                                  _IndexCount * sizeof(uint16))),
	IBMemory(CreateVKMemoryCreator(_Device, IndexBuffer.GetMemoryRequirements(), VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT))
{
	...
	VKBuffer StagingVB(CreateVKBufferCreator(
		_Device, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, _VertexDataSize));
	VKBuffer StagingIB(CreateVKBufferCreator(
		_Device, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT, _IndexCount * sizeof(uint16)));

	VKMemory StagingVBMemory(CreateVKMemoryCreator(_Device, StagingVB.GetMemoryRequirements(),
	                                               VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
	                                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT));
	VKMemory StagingIBMemory(CreateVKMemoryCreator(_Device, StagingIB.GetMemoryRequirements(),
	                                               VK_MEMORY_PROPERTY_HOST_COHERENT_BIT |
	                                               VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT));
