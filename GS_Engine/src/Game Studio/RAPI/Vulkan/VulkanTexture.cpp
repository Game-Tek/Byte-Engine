...
VulkanTexture::VulkanTexture(VulkanRenderDevice* vulkanRenderDevice, const TextureCreateInfo& textureCreateInfo) : Texture(textureCreateInfo)
{
	...
	bufferInfo.size = supportedTextureSize;
	bufferInfo.usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT;
	bufferInfo.sharingMode = VK_SHARING_MODE_EXCLUSIVE;

	VK_CHECK(vkCreateBuffer(vulkanRenderDevice->GetVkDevice(), &bufferInfo, vulkanRenderDevice->GetVkAllocationCallbacks(), &staging_buffer));
...

	{
		...
		if (VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL && to_image_layout == VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL)
		{
