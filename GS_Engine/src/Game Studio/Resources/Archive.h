...
/**
 * \brief Is an interface for serialization or de-serialization of data.
 * Provides methods to read or write data from/to disk.
...
public:
	...
	explicit Archive(const Archive& _Archive) = default;

	explicit Archive(Archive&& _Archive) = default;
	
	~Archive() = default;

	Archive& operator=(const Archive& _Other) = default;
	Archive& operator=(Archive&& _Other) = default;
	 ...
	 */
	INLINE void Write(int8 _In)    const { writeInternal(sizeof(int8), &_In); }
	
	/**
	* \brief Writes an uint8 to disk.
	* \param _In uint8 to be written to disk.
	*/
	INLINE void Write(uint8 _In)   const { writeInternal(sizeof(uint8), &_In); }

	/**
	* \brief Writes an int16 to disk.
	* \param _In int16 to be written to disk.
	*/
	INLINE void Write(int16 _In)   const { writeInternal(sizeof(int16), &_In); }

	/**
	* \brief Writes an uint16 to disk.
	* \param _In uint16 to be written to disk.
	*/
	INLINE void Write(uint16 _In)  const { writeInternal(sizeof(uint16), &_In); }

	/**
	* \brief Writes an int32 to disk.
	* \param _In int32 to be written to disk.
	*/
	INLINE void Write(int32 _In)   const { writeInternal(sizeof(int32), &_In); }
	...
	*/
	...
	/**
	* \brief Writes an int_64 to disk.
	* \param _In int_64 to be written to disk.
	*/
	INLINE void Write(int_64 _In)  const { writeInternal(sizeof(int_64), &_In); }

	/**
	* \brief Writes an uint_64 to disk.
	* \param _In uint_64 to be written to disk.
	*/
	INLINE void Write(uint_64 _In) const { writeInternal(sizeof(uint_64), &_In); }
	...
	*/
	INLINE void Write(const size_t _Size, void* _In) const { writeInternal(_Size, _In); }
	...
	*/
	...
	/**
	* \brief Writes an int_64 to memory from disk.
	* \param _In Pointer to an int_64 the data will be read to.
	*/
	INLINE void Read(int_64* _In)  const { readInternal(sizeof(int_64), _In); }

	/**
	* \brief Writes an uint_64 to memory from disk.
	* \param _In Pointer to an uint_64 the data will be read to.
	*/
	INLINE void Read(uint_64* _In) const { readInternal(sizeof(uint_64), _In); }
	...
	*/
	INLINE void Read(const size_t _Size, void* _Data) const { readInternal(_Size, _Data); }
