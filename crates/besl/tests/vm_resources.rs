...
fn bindings_named_like_their_member_stay_rooted_after_a_store() {
	...
	views
		.write_array_member(0, "near", Value::F32(0.5))
		.expect("Expected near plane write");
	views
		.write_array_member(0, "far", Value::F32(100.0))
	...
	assert_eq!(
		views.read_array_member(1, "near").expect("Expected near plane"),
		Value::F32(0.5)
	);
	assert_eq!(
		views.read_array_member(1, "far").expect("Expected far plane"),
		Value::F32(100.0)
	);
}
