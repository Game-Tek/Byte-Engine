...
impl GamepadSystem {
	pub(super) fn new() -> Result<Self, String> {
		let api = HidApi::new().map_err(|e| format!(
	...
	fn refresh_devices(&mut self) -> Vec<(String, GamepadKind, HidDevice)> {
		...
		if let Err(error) = self.api.refresh_devices() {
			warn!("Failed to refresh HID devices. The most likely cause is that the HID backend could not enumerate devices: {}", error);
			return Vec::new();
		}
		...
		for device_info in self.api.device_list() {
			let kind = match classify_gamepad(device_info.vendor_id(), device_info.product_id(), device_info.product_string()) {
				Some(kind) => kind,
				None => continue,
			};
			...
			let device = match self.api.open_path(device_info.path()) {
				Ok(device) => device,
				Err(error) => {
					warn!("Failed to open HID device. The most likely cause is insufficient permissions or the device being in use: {}", error);
					continue;
				}
			};

			if let Err(error) = device.set_blocking_mode(false) {
				warn!("Failed to set HID device to non-blocking mode. The most likely cause is a platform HID backend limitation: {}", error);
			}
...
impl GamepadDevice {
	...
	fn emit_changes(&mut self, state: GamepadState) -> Vec<GamepadEvent> {
		...
		if (self.state.left_stick.x - state.left_stick.x).abs() > STICK_EPSILON
			...
			events.push(GamepadEvent::new(self.device_handle, TriggerReference::Name("Gamepad.LeftStick"), Value::Vector2(state.left_stick)));
		}
		...
		if (self.state.right_stick.x - state.right_stick.x).abs() > STICK_EPSILON
			...
			events.push(GamepadEvent::new(self.device_handle, TriggerReference::Name("Gamepad.RightStick"), Value::Vector2(state.right_stick)));
		}

		if (self.state.left_trigger - state.left_trigger).abs() > TRIGGER_EPSILON {
			events.push(GamepadEvent::new(self.device_handle, TriggerReference::Name("Gamepad.LeftTrigger"), Value::Float(state.left_trigger)));
		}

		if (self.state.right_trigger - state.right_trigger).abs() > TRIGGER_EPSILON {
			events.push(GamepadEvent::new(self.device_handle, TriggerReference::Name("Gamepad.RightTrigger"), Value::Float(state.right_trigger)));
		}
		...
		for (mask, name) in BUTTON_TRIGGERS {
			...
			if previous != current {
				events.push(GamepadEvent::new(self.device_handle, TriggerReference::Name(name), Value::Bool(current)));
			}
		}
...
fn parse_dualshock4(report: &[u8]) -> Option<GamepadState> {
	...
	if buttons & 0x10 != 0 { mask |= BUTTON_X; }
	if buttons & 0x20 != 0 { mask |= BUTTON_A; }
	if buttons & 0x40 != 0 { mask |= BUTTON_B; }
	if buttons & 0x80 != 0 { mask |= BUTTON_Y; }

	if buttons2 & 0x01 != 0 { mask |= BUTTON_LEFT_BUMPER; }
	if buttons2 & 0x02 != 0 { mask |= BUTTON_RIGHT_BUMPER; }
	if buttons2 & 0x10 != 0 { mask |= BUTTON_SELECT; }
	if buttons2 & 0x20 != 0 { mask |= BUTTON_START; }
	if buttons2 & 0x40 != 0 { mask |= BUTTON_LEFT_STICK; }
	if buttons2 & 0x80 != 0 { mask |= BUTTON_RIGHT_STICK; }

	if buttons3 & 0x01 != 0 { mask |= BUTTON_GUIDE; }
...
fn parse_xbox(report: &[u8]) -> Option<GamepadState> {
	let report = if report.first().copied() == Some(0x01) { &report[1..] } else { report };
	...
	if buttons & 0x0001 != 0 { mask |= BUTTON_DPAD_UP; }
	if buttons & 0x0002 != 0 { mask |= BUTTON_DPAD_DOWN; }
	if buttons & 0x0004 != 0 { mask |= BUTTON_DPAD_LEFT; }
	if buttons & 0x0008 != 0 { mask |= BUTTON_DPAD_RIGHT; }

	if buttons & 0x0010 != 0 { mask |= BUTTON_START; }
	if buttons & 0x0020 != 0 { mask |= BUTTON_SELECT; }
	if buttons & 0x0040 != 0 { mask |= BUTTON_LEFT_STICK; }
	if buttons & 0x0080 != 0 { mask |= BUTTON_RIGHT_STICK; }

	if buttons & 0x0100 != 0 { mask |= BUTTON_LEFT_BUMPER; }
	if buttons & 0x0200 != 0 { mask |= BUTTON_RIGHT_BUMPER; }
	if buttons & 0x0400 != 0 { mask |= BUTTON_GUIDE; }

	if buttons & 0x1000 != 0 { mask |= BUTTON_A; }
	if buttons & 0x2000 != 0 { mask |= BUTTON_B; }
	if buttons & 0x4000 != 0 { mask |= BUTTON_X; }
	if buttons & 0x8000 != 0 { mask |= BUTTON_Y; }
