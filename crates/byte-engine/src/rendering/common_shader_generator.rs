...
mod tests {
	...
	/// Verifies that generated fixed-ID compute bindings execute through the Metal GHI.
	#[cfg(target_os = "macos")]
	#[test]
	fn generated_msl_fixed_slot_executes_through_ghi() {
		use ghi::{
		...
		let program = besl::compile_to_besl(
			"Result: struct { value: u32, } result: descriptor<Result, 9, write, device>; main: fn () -> void { result.value = 4660; }",
			None,
		)
		...
		let mut context = instance
			.create_device(
				features,
				&mut [(ghi::QueueSelection::new(ghi::WorkloadTypes::COMPUTE), &mut queue_handle)],
			)
		...
		let shader = context
			.create_shader(
				...
				ghi::shader::Sources::MTL {
					source: &source,
					entry_point: resource_management::shader::besl::backends::msl::MSL_ENTRY_POINT,
				},
	...
	/// Verifies that stage reachability does not change the authored Metal resource ABI.
	#[cfg(target_os = "macos")]
	#[test]
	fn generated_msl_stage_reachability_preserves_resource_abi() {
		use ghi::{
			command_buffer::{
		...
		let vertex_program = besl::compile_to_besl(
			"Color: struct { value: vec4f, } color: descriptor<Color, 0, read, constant>; Scale: struct { value: f32, } scale: descriptor<Scale, 9, read, constant>; in_position: input<vec3f, 0>; position: output<vec4f, 0>; main: fn () -> void { position = vec4f(in_position.x * scale.value, in_position.y * scale.value, in_position.z, 1.0); }",
			None,
		)
		...
		let mut context = instance
			.create_device(
				features,
				&mut [(ghi::QueueSelection::new(ghi::WorkloadTypes::RASTER), &mut queue_handle)],
			)
		...
		*context.get_mut_buffer_slice(index_buffer) = [0, 2, 1];
		...
		let pipeline = context.create_raster_pipeline(ghi::pipelines::raster::Builder::new(
			...
			&vertex_layout,
			&[
				ghi::pipelines::ShaderParameter::new(&vertex_shader, ghi::ShaderTypes::Vertex),
				ghi::pipelines::ShaderParameter::new(&fragment_shader, ghi::ShaderTypes::Fragment),
			],
			...
			.execute(Some(FrameRequest::new(0, signal)), &[], signal, |execution| {
				execution.record(command_buffer, |recording| {
					let attachments = [ghi::AttachmentInformation::new(
						...
						ghi::ClearValue::Color(utils::RGBA {
							r: 0.0,
							g: 0.0,
							b: 0.0,
							a: 1.0,
