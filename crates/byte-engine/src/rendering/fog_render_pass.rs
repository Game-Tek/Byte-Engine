...
impl FogRenderPass {
	pub fn new(ghi: &mut ghi::GHI, visibility_descriptor_set_template: &ghi::DescriptorSetTemplateHandle, depth_target: ghi::ImageHandle, out_target: ghi::ImageHandle) -> Self {
		...
		let descriptor_set_template = ghi.create_descriptor_set_template(Some("Fog Rendering Set Layout"), &[depth_map_binding_template.clone(), out_map_binding_template.clone()]);

		let pipeline_layout = ghi.create_pipeline_layout(&[visibility_descriptor_set_template.clone(), descriptor_set_template], &[]);

		let descriptor_set = ghi.create_descriptor_set(Some("Fog Rendering Descriptor Set"), &descriptor_set_template);
		...
		let shader = ghi.create_shader(Some("Fog Rendering Shader"), ghi::ShaderSource::GLSL(Self::make_shader()), ghi::ShaderTypes::Compute, &[depth_map_binding_template.into_shader_binding_descriptor(1, ghi::AccessPolicies::READ), out_map_binding_template.into_shader_binding_descriptor(1, ghi::AccessPolicies::READ_WRITE)]).unwrap();
	...
	fn make_shader() -> String {
		...
		vec3 c = imageLoad(out_target, pixel_coordinates).rgb;
		...
		for (int i = 0; i < 32; ++i) {
			float a = 1.0 - float(i) / 32.0;

			if (depth < a) {
				float cos_theta = dot(viewDir, normalize(vec3(0.0, 1.0, 0.0)));
				float rayleigh_phase = rayleigh_phase(cos_theta);
				float henyey_greenstein_phase = henyey_greenstein_phase(cos_theta, 0.76);
				vec3 total_rayleigh = total_rayleigh(primaries, refractive_index, depolarization_factor, num_molecules);
				vec3 total_mie = total_mie(primaries, mieKCoefficient, 0.76, 0.0001);
				c += total_rayleigh * rayleigh_phase + total_mie * henyey_greenstein_phase;
			}
		}

		imageStore(out_target, pixel_coordinates, vec4(c, 1.0));
		"#;
		...
		root.add(vec![
			...
			besl::parser::Node::binding("out_target", besl::parser::Node::image("rgba16"), 1, 1, true, true),
...
impl RenderPass for FogRenderPass {
	...
	fn record(&self, command_buffer_recording: &mut ghi::CommandBufferRecording, extent: Extent) {
		command_buffer_recording.start_region("Fog Render Pass");
