...
impl VisibilityPass {
	pub fn new(device: &mut ghi::Device, pipeline_layout: ghi::PipelineLayoutHandle, descriptor_set: ghi::DescriptorSetHandle, primitive_index: ghi::ImageHandle, instance_id: ghi::ImageHandle, depth_target: ghi::ImageHandle) -> Self {
		...
		let visibility_pass_mesh_shader = device.create_shader(Some("Visibility Pass Mesh Shader"), ghi::ShaderSource::SPIRV(visibility_mesh_shader_artifact.borrow().into()), ghi::ShaderTypes::Mesh,
			[
		...
		let visibility_pass_fragment_shader = device.create_shader(Some("Visibility Pass Fragment Shader"), ghi::ShaderSource::SPIRV(visibility_fragment_shader_artifact.borrow().into()), ghi::ShaderTypes::Fragment, []).expect("Failed to create shader");
		...
		let visibility_pass_pipeline = device.create_raster_pipeline(raster_pipeline::Builder::new(pipeline_layout, &[], &visibility_pass_shaders, &attachments));
	...
	pub(super) fn prepare(&self, frame: &mut ghi::Frame, viewport: &Viewport, instances: &[Instance]) -> impl RenderPassFunction {
...
impl MaterialCountPass {
	fn new(device: &mut ghi::Device, pipeline_layout: ghi::PipelineLayoutHandle, descriptor_set: ghi::DescriptorSetHandle, visibility_pass_descriptor_set: ghi::DescriptorSetHandle) -> Self {
		let material_count_shader_artifact = glsl::compile(&get_material_count_source(), "Material Count Pass Compute Shader").unwrap();

		let material_count_shader = device.create_shader(Some("Material Count Pass Compute Shader"), ghi::ShaderSource::SPIRV(material_count_shader_artifact.borrow().into()), ghi::ShaderTypes::Compute,
			[
				MESH_DATA_BINDING.into_shader_binding_descriptor(0, ghi::AccessPolicies::READ),
		...
		let material_count_pipeline = device.create_compute_pipeline(pipeline_layout, ghi::ShaderParameter::new(&material_count_shader, ghi::ShaderTypes::Compute));

		let material_count_buffer = device.create_buffer(Some("Material Count"), ghi::Uses::Storage | ghi::Uses::TransferDestination, ghi::DeviceAccesses::HostOnly);
...
impl MaterialOffsetPass {
	fn new(device: &mut ghi::Device, pipeline_layout: ghi::PipelineLayoutHandle, descriptor_set: ghi::DescriptorSetHandle, visibility_pass_descriptor_set: ghi::DescriptorSetHandle) -> Self {
		...
		let material_offset_shader = device.create_shader(Some("Material Offset Pass Compute Shader"), ghi::ShaderSource::SPIRV(material_offset_shader_artifact.borrow().into()), ghi::ShaderTypes::Compute,
			[
		...
		let material_offset_pipeline = device.create_compute_pipeline(pipeline_layout, ghi::ShaderParameter::new(&material_offset_shader, ghi::ShaderTypes::Compute,));

		let material_evaluation_dispatches = device.create_buffer(Some("Material Evaluation Dipatches"), ghi::Uses::Storage | ghi::Uses::TransferDestination | ghi::Uses::Indirect, ghi::DeviceAccesses::DeviceOnly);
		let material_offset_buffer = device.create_buffer(Some("Material Offset"), ghi::Uses::Storage | ghi::Uses::TransferDestination, ghi::DeviceAccesses::DeviceOnly);
		let material_offset_scratch_buffer = device.create_buffer(Some("Material Offset Scratch"), ghi::Uses::Storage | ghi::Uses::TransferDestination, ghi::DeviceAccesses::DeviceOnly);
	...
	fn prepare(&self) -> impl RenderPassFunction {
		...
		move |c, _| {
			...
			c.clear_buffers(&[material_offset_buffer.into(), material_offset_scratch_buffer.into(), material_evaluation_dispatches.into()]);
...
impl PixelMappingPass {
	fn new(device: &mut ghi::Device, pipeline_layout: ghi::PipelineLayoutHandle, descriptor_set: ghi::DescriptorSetHandle, visibility_passes_descriptor_set: ghi::DescriptorSetHandle) -> Self {
		let pixel_mapping_shader_artifact = glsl::compile(&get_pixel_mapping_source(), "Pixel Mapping Pass Compute Shader").unwrap();

		let pixel_mapping_shader = device.create_shader(Some("Pixel Mapping Pass Compute Shader"), ghi::ShaderSource::SPIRV(pixel_mapping_shader_artifact.borrow().into()), ghi::ShaderTypes::Compute,
			[
				MESH_DATA_BINDING.into_shader_binding_descriptor(0, ghi::AccessPolicies::READ),
...
impl MaterialEvaluationPass {
	...
	fn prepare(&self, frame: &mut ghi::Frame, viewport: &Viewport, opaque_materials: &[(&str, usize, ghi::PipelineHandle)], transparent_materials: &[(&str, usize, ghi::PipelineHandle)]) -> impl RenderPassFunction {
		...
		let opaque_materials = opaque_materials.iter().map(|e| (e.0.to_string(), e.1, e.2)).collect::<Vec<_>>();
		let transparent_materials = transparent_materials.iter().map(|e| (e.0.to_string(), e.1, e.2)).collect::<Vec<_>>();
		...
		move |c, t| {
			...
			for (name, index, pipeline) in &opaque_materials {
				...
				c.bind_descriptor_sets(&[base_descriptor_set, visibility_descriptor_set, material_evaluation_descriptor_set]);
...
impl VisibilityPipelineRenderPass {
	pub fn new(
		...
		let visibility_pass = VisibilityPass::new(device, base_pipeline_layout, base_descriptor_set, primitive_index, instance_id, depth);
		let material_count_pass = MaterialCountPass::new(device, visibility_pipeline_layout, base_descriptor_set, visibility_descriptor_set);
		let material_offset_pass = MaterialOffsetPass::new(device, visibility_pipeline_layout, base_descriptor_set, visibility_descriptor_set);
		let pixel_mapping_pass = PixelMappingPass::new(device, visibility_pipeline_layout, base_descriptor_set, visibility_descriptor_set,);
