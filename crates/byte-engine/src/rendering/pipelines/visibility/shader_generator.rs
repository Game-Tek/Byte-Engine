	...
	fn material_evaluation_msl_source_compiles_for_metal() {
		...
		assert!(
			source.contains("texture2d<float, access::write> lit_map [[id(0)]];")
				...
				&& source.contains("texture2d<float> ao [[id(10)]];")
				&& source.contains("sampler ao_sampler [[id(11)]];")
				&& source.contains("texture2d<float> visibility_depth [[id(14)]];")
				&& source.contains("sampler visibility_depth_sampler [[id(15)]];")
