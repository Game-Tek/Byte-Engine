...
const UI_VERTEX_LAYOUT: [ghi::VertexElement; 1] = [ghi::VertexElement::new(
    "POSITION",
...
impl UiRenderPass {
    ...
    pub fn new(render_pass_builder: &mut RenderPassBuilder, data: UiRenderData) -> Self {
        ...
        let pipeline_layout = device
            .create_pipeline_layout(&[], &[ghi::PushConstantRange::new(0, push_constant_size)]);
        ...
        let attachments = [ghi::PipelineAttachmentInformation::new(
...
impl RenderPass for UiRenderPass {
    fn prepare(
        &mut self,
        _frame: &mut ghi::Frame,
        viewport: &Viewport,
    ) -> Option<RenderPassReturn> {
...
fn create_vertex_shader(device: &mut ghi::Device) -> ghi::ShaderHandle {
    ...
    let shader_scope = ParserNode::scope(
        "Shader",
        vec![push_constant, position_input, color_output, main],
    );
    root.add(vec![CommonShaderScope::new(), shader_scope]);

    let root_node = besl::lex(root).expect(
        "Failed to lex the UI vertex shader. The most likely cause is invalid BESL syntax.",
    );
    ...
    let generated = shader_generator.generate(&ShaderGenerationSettings::vertex(), &main_node).expect(
		"Failed to generate UI vertex shader SPIR-V. The most likely cause is invalid GLSL emitted from BESL.",
	);
    ...
    device
		...
		.expect(
			"Failed to create the UI vertex shader. The most likely cause is an incompatible shader interface.",
		)
}
...
fn create_fragment_shader(device: &mut ghi::Device) -> ghi::ShaderHandle {
    ...
    let root_node = besl::lex(root).expect(
        "Failed to lex the UI fragment shader. The most likely cause is invalid BESL syntax.",
    );
    let main_node = root_node.get_main().expect(
		"Failed to find the UI fragment entry point. The most likely cause is that the shader main function was not generated.",
	);
    let generated = shader_generator.generate(&ShaderGenerationSettings::fragment(), &main_node).expect(
		"Failed to generate UI fragment shader SPIR-V. The most likely cause is invalid GLSL emitted from BESL.",
	);
    ...
    device
		...
		.expect(
			"Failed to create the UI fragment shader. The most likely cause is an incompatible shader interface.",
		)
}
...
mod tests {
    ...
    impl Component for TestComponent {
        fn render(&self, ctx: &mut impl Context) {
            ...
            ctx.element(&BaseContainer::new(
                ContainerSettings::default().size(64.into()),
            ));
        }
    }
