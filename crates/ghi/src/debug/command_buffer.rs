use utils::Extent;
use crate::{
	AttachmentInformation, BaseBufferHandle, BindingTables, BottomLevelAccelerationStructureBuild,
	BufferDescriptor, BufferHandle, ClearValue, CommandBufferHandle, DescriptorSetHandle,
	DispatchExtent, FrameKey, ImageHandle, Layouts, MeshHandle, PipelineHandle,
	PipelineLayoutHandle, PresentKey, RGBAu8, SwapchainHandle, SynchronizerHandle,
...
impl<'a> CommandBufferRecording<'a> {
	...
	pub fn build_top_level_acceleration_structure(&mut self, _acceleration_structure_build: &TopLevelAccelerationStructureBuild) {
	}

	pub fn build_bottom_level_acceleration_structures(&mut self, _acceleration_structure_builds: &[BottomLevelAccelerationStructureBuild]) {
	}
	...
	pub fn blit_image(&mut self, _source_image: ImageHandle, _source_layout: Layouts, _destination_image: ImageHandle, _destination_layout: Layouts) {
	}

	pub fn copy_to_swapchain(&mut self, _source_texture_handle: ImageHandle, _present_image_index: PresentKey, _swapchain_handle: SwapchainHandle) {
	}
	...
	pub fn execute(self, _wait_for_synchronizer_handles: &[SynchronizerHandle], _signal_synchronizer_handles: &[SynchronizerHandle], _presentations: &[PresentKey], _execution_synchronizer_handle: SynchronizerHandle) {
	}
	...
	pub fn draw_indexed(&mut self, _index_count: u32, _instance_count: u32, _first_index: u32, _vertex_offset: i32, _first_instance: u32) {
	}
