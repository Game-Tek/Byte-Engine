...
use crate::{
	...
	PresentationModes, PushConstantRange, QueueHandle, SamplerAddressingModes, SamplerHandle,
...
impl Device {
	...
	pub fn create_allocation(&mut self, _size: usize, _resource_uses: Uses, _resource_device_accesses: DeviceAccesses) -> AllocationHandle {
		AllocationHandle(0)
	}

	pub fn add_mesh_from_vertices_and_indices(&mut self, _vertex_count: u32, _index_count: u32, _vertices: &[u8], _indices: &[u8], _vertex_layout: &[VertexElement]) -> MeshHandle {
		MeshHandle(0)
	}

	pub fn create_shader(&mut self, _name: Option<&str>, _shader_source_type: ShaderSource, _stage: ShaderTypes, _shader_binding_descriptors: impl IntoIterator<Item = ShaderBindingDescriptor>) -> Result<ShaderHandle, ()> {
		Ok(ShaderHandle(0))
	}

	pub fn create_descriptor_set_template(&mut self, _name: Option<&str>, _binding_templates: &[DescriptorSetBindingTemplate]) -> DescriptorSetTemplateHandle {
		DescriptorSetTemplateHandle(0)
	}

	pub fn create_descriptor_set(&mut self, _name: Option<&str>, _descriptor_set_template_handle: &DescriptorSetTemplateHandle) -> DescriptorSetHandle {
		DescriptorSetHandle(0)
	}

	pub fn create_descriptor_binding(&mut self, _descriptor_set: DescriptorSetHandle, _binding_constructor: BindingConstructor) -> DescriptorSetBindingHandle {
		DescriptorSetBindingHandle(0)
	}

	pub fn create_pipeline_layout(&mut self, _descriptor_set_template_handles: &[DescriptorSetTemplateHandle], _push_constant_ranges: &[PushConstantRange]) -> PipelineLayoutHandle {
	...
	pub fn create_compute_pipeline(&mut self, _pipeline_layout_handle: PipelineLayoutHandle, _shader_parameter: ShaderParameter) -> PipelineHandle {
		PipelineHandle(0)
	}

	pub fn create_ray_tracing_pipeline(&mut self, _pipeline_layout_handle: PipelineLayoutHandle, _shaders: &[ShaderParameter]) -> PipelineHandle {
		PipelineHandle(0)
	}
	...
	pub fn create_buffer<T: Copy>(&mut self, _name: Option<&str>, _resource_uses: Uses, _device_accesses: DeviceAccesses) -> BufferHandle<T> {
		BufferHandle(0, std::marker::PhantomData)
	}

	pub fn create_dynamic_buffer<T: Copy>(&mut self, _name: Option<&str>, _resource_uses: Uses, _device_accesses: DeviceAccesses) -> DynamicBufferHandle<T> {
	...
	pub fn create_image(&mut self, _name: Option<&str>, _extent: Extent, _format: Formats, _resource_uses: Uses, _device_accesses: DeviceAccesses, _use_case: UseCases, _array_layers: Option<NonZeroU32>) -> ImageHandle {
		ImageHandle(0)
	}
	...
	pub fn create_sampler(&mut self, _filtering_mode: FilteringModes, _reduction_mode: SamplingReductionModes, _mip_map_mode: FilteringModes, _addressing_mode: SamplerAddressingModes, _anisotropy: Option<f32>, _min_lod: f32, _max_lod: f32) -> SamplerHandle {
		SamplerHandle(0)
	}
	...
	pub fn create_acceleration_structure_instance_buffer(&mut self, _name: Option<&str>, _max_instance_count: u32) -> BaseBufferHandle {
		BaseBufferHandle(0)
	}
	...
	pub fn write_instance(&mut self, _instances_buffer_handle: BaseBufferHandle, _instance_index: usize, _transform: [[f32; 4]; 3], _custom_index: u16, _mask: u8, _sbt_record_offset: usize, _acceleration_structure: BottomLevelAccelerationStructureHandle) {
	}

	pub fn write_sbt_entry(&mut self, _sbt_buffer_handle: BaseBufferHandle, _sbt_record_offset: usize, _pipeline_handle: PipelineHandle, _shader_handle: ShaderHandle) {
	}

	pub fn bind_to_window(&mut self, _window_os_handles: &window::Handles, _presentation_mode: PresentationModes, _fallback_extent: Extent) -> SwapchainHandle {
		SwapchainHandle(0)
	}
