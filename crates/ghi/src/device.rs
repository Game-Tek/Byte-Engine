...
use crate::{image, raster_pipeline, sampler, window, AllocationHandle, BaseBufferHandle, BindingConstructor, BottomLevelAccelerationStructure, BottomLevelAccelerationStructureHandle, BufferHandle, CommandBufferHandle, CommandBufferRecording, DescriptorSetBindingHandle, DescriptorSetBindingTemplate, DescriptorSetHandle, DescriptorSetTemplateHandle, DescriptorWrite, DeviceAccesses, DynamicBufferHandle, FilteringModes, Formats, Frame, FrameKey, ImageHandle, MeshHandle, PipelineHandle, PipelineLayoutHandle, PresentKey, PresentationModes, PushConstantRange, QueueHandle, SamplerAddressingModes, SamplerHandle, SamplingReductionModes, ShaderBindingDescriptor, ShaderHandle, ShaderParameter, ShaderSource, ShaderTypes, SwapchainHandle, SynchronizerHandle, TextureCopyHandle, TopLevelAccelerationStructureHandle, UseCases, Uses, VertexElement};
...
pub trait Device where Self: Sized {
	...
	fn start_frame(&mut self, index: u32, synchronizer_handle: SynchronizerHandle) -> Frame;
