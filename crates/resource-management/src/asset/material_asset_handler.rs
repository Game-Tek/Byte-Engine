...
use crate::{
    ...
    resource,
    resources::material::{
        Binding, MaterialModel, ParameterModel, RenderModel, Shader, ShaderInterface, ValueModel,
...
pub trait ProgramGenerator: Send + Sync {
    ...
    fn transform<'a>(
        &self,
        node: besl::parser::Node<'a>,
        material: &'a json::Object,
    ) -> besl::parser::Node<'a>;
}
...
impl Asset for MaterialAsset {
    ...
    fn load<'a>(
        ...
        Box::pin(async move {
            ...
            if is_material {
                let material_domain = asset["domain"]
                    .as_str()
                    .ok_or(
                        "Domain not found. The material JSON is missing a domain field."
                            .to_string(),
                    )
                    .or_else(|e| {
                        debug!("{}", e);
                ...
                let asset_shaders = match asset["shaders"].as_object() {
                    ...
                    None => {
                        return Err(
                            "Shaders not found. The material JSON is missing shader definitions."
                                .to_string(),
                        );
                    }
                };
                ...
                let asset_variables = match asset["variables"].as_array() {
                    Some(v) => v,
                    None => {
                        return Err("Variables not found. The material JSON is missing variable definitions.".to_string());
                    }
                };
                ...
                for v in asset_variables.iter() {
                    ...
                    let value =
                        resolve_value(asset_manager, storage_backend, &data_type, &value).await?;
                    values.push(value);
                }
                ...
                for v in material_repr.parameters.iter() {
                    let value = match asset["variables"].as_array() {
                        Some(variables) => {
                            match variables
                                .iter()
                                .find(|v2| v2["name"].as_str().unwrap() == v.name)
                            {
                                Some(v) => v["value"].as_str().unwrap().to_string(),
                                None => {
                                    return Err("Variable not found. The variant JSON is missing an override.".to_string());
                                }
                            }
                        }
                        None => {
                            return Err(
                                "Variable not found. The variant JSON is missing overrides."
                                    .to_string(),
                            );
                        }
                    };
                    ...
                    let resolved =
                        resolve_value(asset_manager, storage_backend, &v.r#type, &value).await?;
...
impl AssetHandler for MaterialAssetHandler {
    ...
    fn load<'a>(
        ...
        Box::pin(async move {
            ...
            let asset_json = json::from_str(
                std::str::from_utf8(&data).or_else(|_| Err(LoadErrors::FailedToProcess))?,
            )
...
fn compile_shader(
    ...
    let shader_program = SPIRVShaderGenerator::new()
    ...
    let interface = ShaderInterface {
        workgroup_size: shader_program
            .extent()
            .map(|e| (e.width(), e.height(), e.depth())),
...
async fn transform_shader(
    ...
    let shader_code = std::str::from_utf8(&arlp)
        .map_err(|_| {
            "Failed to decode shader source. The shader file is not valid UTF-8.".to_string()
        })?
        .to_string();
    ...
    .map_err(|_| {
        "Failed to compile shader. The shader source likely contains errors.".to_string()
    })?;

    let r = storage_backend
        .store(&ProcessedAsset::new(path, shader), &result_shader_bytes)
        .or(Err(
            "Failed to store shader resource. The storage backend likely rejected the write."
                .to_string(),
        ))?;
...
pub mod tests {
    ...
    use crate::{
        asset::{
            ...
            material_asset_handler::MaterialAssetHandler,
    ...
    impl ProgramGenerator for RootTestShaderGenerator {
        fn transform<'a>(
            &self,
            mut root: besl::parser::Node<'a>,
            material: &'a json::Object,
        ) -> besl::parser::Node<'a> {
            let material_struct = besl::parser::Node::buffer(
                "Material",
                vec![besl::parser::Node::member("color", "vec4f")],
            );

            let sample_function = besl::parser::Node::function(
                "sample_",
                vec![besl::parser::Node::member("t", "u32")],
                "void",
                vec![],
            );
    ...
    impl ProgramGenerator for MidTestShaderGenerator {
        fn transform<'a>(
            ...
            let binding = besl::parser::Node::binding(
                "materials",
                besl::parser::Node::buffer(
                    "Materials",
                    vec![besl::parser::Node::member("materials", "Material[16]")],
                ),
                0,
                0,
                true,
    ...
    impl ProgramGenerator for LeafTestShaderGenerator {
        fn transform<'a>(
            ...
            let push_constant =
                besl::parser::Node::push_constant(vec![besl::parser::Node::member(
                    "material_index",
                    "u32",
                )]);
