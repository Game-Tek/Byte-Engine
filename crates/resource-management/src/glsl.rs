...
pub fn process_glslc_error<'a>(
    ...
    let errors = error_string
        ...
        .map(|(error_line_number_string, error)| {
            (
                error_line_number_string.trim().parse::<usize>().unwrap() - 1,
                error,
            )
        })
        .collect::<Vec<_>>();
    ...
    let mut errors_by_line_number =
        std::collections::HashMap::<usize, Vec<(&'a str, &'a str)>>::new();
...
pub fn pretty_format_glsl_error_lines(error_lines: &[Line]) -> String {
    let max_error_line_number = error_lines
        .iter()
        .map(|error_line| error_line.line_number)
        .max()
        .unwrap_or(0);
...
pub fn pretty_format_glslang_error_string(
    error_string: &str,
    shader_name: &str,
    source_code: &str,
) -> String {
    let error_lines = process_glslc_error(shader_name, source_code, error_string);
    let error_string = pretty_format_glslang_errors(&error_lines, source_code)
        .unwrap_or_else(|| error_string.to_string());
    error_string
}
