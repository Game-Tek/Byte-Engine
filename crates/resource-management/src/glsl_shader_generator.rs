...
impl GLSLShaderGenerator {
    ...
    fn generate_glsl_header_block(
        ...
        match compilation_settings.stage {
            ...
            Stages::Mesh {
                ...
                glsl_block.push_str(&format!(
                    "layout(location=0) perprimitiveEXT out uint out_instance_index[{}];\n",
                    maximum_primitives
                ));
                glsl_block.push_str(&format!(
                    "layout(location=1) perprimitiveEXT out uint out_primitive_index[{}];\n",
                    maximum_primitives
                ));
                glsl_block.push_str(&format!(
        ...
        match compilation_settings.matrix_layout {
            MatrixLayouts::RowMajor => {
                glsl_block.push_str("layout(row_major) uniform;layout(row_major) buffer;\n")
            }
            MatrixLayouts::ColumnMajor => {
                glsl_block.push_str("layout(column_major) uniform;layout(column_major) buffer;\n")
            }
        }
...
mod tests {
    ...
    fn bindings() {
        ...
        assert_string_contains!(
            shader,
            "layout(set=0,binding=0,scalar) buffer _buff{float member;}buff;"
        );
        assert_string_contains!(
            shader,
            "layout(set=0,binding=1,r8) writeonly uniform image2D image;"
        );
    ...
    fn input() {
        ...
        assert_string_contains!(
            shader,
            "layout(location=0)in vec3 color;void main(){color;}"
        );
    }
    ...
    fn output() {
        ...
        assert_string_contains!(
            shader,
            "layout(location=0)out vec3 color;void main(){color;}"
        );
    }
    ...
    fn test_multi_language_raw_code() {
        ...
        assert!(
            !shader.contains("float4"),
            "GLSL shader should not contain HLSL code"
        );
    }
}
