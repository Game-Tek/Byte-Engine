...
impl HLSLShaderGenerator {
    ...
    pub fn generate(
        ...
        if !matches!(
            main_function_node.borrow().node(),
            besl::Nodes::Function { .. }
        ) {
    ...
    fn emit_node_string(&mut self, string: &mut String, this_node: &besl::NodeReference) {
        ...
        match node.node() {
            ...
            besl::Nodes::Function {
                ...
                string.push_str(Self::translate_type(
                    &return_type.borrow().get_name().unwrap(),
                ));
                ...
                match r#type {
                    besl::BindingTypes::Buffer { members } => {
                        ...
                        if use_cbuffer {
                            ...
                            string.push_str(&format!(
                                " : register({}{}, space{});",
                                register_letter, register_index, set
                            ));
                            if !self.minified {
                                string.push('\n');
                            }
                    ...
                    besl::BindingTypes::Image { format } => {
                        ...
                        string
                            .push_str(&format!(" : register(u{}, space{});", register_index, set));
                        if !self.minified {
                            string.push('\n');
                        }
                    }
                    besl::BindingTypes::CombinedImageSampler { format } => {
                        ...
                        string
                            .push_str(&format!(" : register(t{}, space{});", register_index, set));
                        if !self.minified {
                            string.push('\n');
                        }
                        ...
                        string
                            .push_str(&format!(" : register(s{}, space{});", register_index, set));
                        if !self.minified {
                            string.push('\n');
                        }
                    }
    ...
    fn generate_hlsl_header_block(
        ...
        match compilation_settings.matrix_layout {
            ...
            MatrixLayouts::ColumnMajor => {
                hlsl_block.push_str("#pragma pack_matrix(column_major)\n")
            }
        }
...
mod tests {
    ...
    fn bindings() {
        ...
        assert_string_contains!(
            shader,
            "RWStructuredBuffer<_buff> buff : register(u0, space0);"
        );
        ...
        assert_string_contains!(
            shader,
            "Texture2D<float4> texture : register(t100, space1);"
        );
        assert_string_contains!(
            shader,
            "SamplerState texture_sampler : register(s100, space1);"
        );
    ...
    fn specializtions() {
        ...
        assert_string_contains!(
            shader,
            "const float3 color=float3(color_x,color_y,color_z);"
        );
    ...
    fn test_multi_language_raw_code() {
        ...
        assert!(
            !shader.contains("gl_Position"),
            "HLSL shader should not contain GLSL code"
        );
    }
}
