...
fn compile_msl_to_metallib(msl_source: &str, name: &str) -> Result<Box<[u8]>, String> {
        ...
        .args([
            ...
            source_path.to_str().ok_or_else(|| {
                error(
                    ...
                    "The temporary file path was not valid UTF-8",
                )
            })?,
            "-o",
            air_path.to_str().ok_or_else(|| {
                error(
                    "Failed to compile MSL shader",
                    "The temporary file path was not valid UTF-8",
                )
    ...
    let metallib_output = Command::new("xcrun")
        .args([
            ...
            "-o",
            metallib_path.to_str().ok_or_else(|| {
                error(
                    "Failed to link Metal library",
                    "The temporary file path was not valid UTF-8",
                )
    ...
    let binary = fs::read(&metallib_path).map_err(|_| {
        error(
            "Failed to read compiled Metal library",
            "The metallib output was not created",
        )
    })?;
