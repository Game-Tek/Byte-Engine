...
impl MSLShaderGenerator {
    ...
    fn emit_node_string(&mut self, string: &mut String, this_node: &besl::NodeReference) {
        ...
        match node.node() {
            ...
            besl::Nodes::Input {
                ...
                string.push_str(&format!(
                    "{} {} [[attribute({})]];{break_char}",
                    type_name, name, location
                ));
            }
            besl::Nodes::Output {
                name,
            ...
            besl::Nodes::Binding {
                ...
                match r#type {
                    ...
                    besl::BindingTypes::Image { format } => {
                        ...
                        string
                            .push_str(&format!("texture2d<{}, {}> {}", element_type, access, name));
    ...
    fn generate_msl_header_block(
        ...
        match compilation_settings.stage {
            Stages::Compute { .. } => {
                msl_block
                    .push_str("// Note: Metal threadgroup sizes are set on the pipeline state.\n");
            }
            Stages::Mesh { .. } => {
                msl_block
                    .push_str("// Note: Metal mesh shader configuration requires manual setup.\n");
            }
            _ => {}
        }
...
mod tests {
    ...
    fn specializtions() {
        ...
        assert_string_contains!(
            shader,
            "constant float color_x [[function_constant(0)]] = 1.0f;"
        );
        assert_string_contains!(
            shader,
            "constant float color_y [[function_constant(1)]] = 1.0f;"
        );
        assert_string_contains!(
            shader,
            "constant float color_z [[function_constant(2)]] = 1.0f;"
        );
        assert_string_contains!(
            shader,
            "constant float3 color=float3(color_x,color_y,color_z);"
        );
    ...
    fn push_constant() {
        ...
        assert_string_contains!(
            shader,
            "constant PushConstant& push_constant [[buffer(0)]];"
        );
    ...
    fn test_multi_language_raw_code() {
        ...
        assert!(
            ...
            "MSL shader should not contain GLSL code"
        );
    }
}
