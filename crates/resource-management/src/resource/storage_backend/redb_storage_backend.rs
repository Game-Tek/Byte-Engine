...
use crate::{
    asset,
    resource::{
        reader::redb::FileResourceReader, resource_handler::MultiResourceReader, ResourceId,
    },
...
const RESOURCES_TABLE: redb::TableDefinition<[u8; 16], &[u8]> =
...
impl ReadStorageBackend for RedbStorageBackend {
    ...
    fn read<'s, 'a, 'b>(
        &'s self,
        id: asset::ResourceId<'b>,
    ) -> Option<(SerializableResource, MultiResourceReader)> {
        let read = self.db.begin_read().unwrap();
        let table = read.open_table(RESOURCES_TABLE).unwrap();
    ...
    fn query<'a>(
        &'a self,
        query: Query<'a>,
    ) -> Result<Vec<(SerializableResource, MultiResourceReader)>, ()> {
        let base_path = self.base_path.clone();
        ...
        let resources = table
            ...
            .filter_map(|d| {
                ...
                let class = query
                    .class
                    .map_or(false, |c| c.contains(&resource.class.as_str()));
