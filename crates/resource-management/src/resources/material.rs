...
impl<'de> Solver<'de, Reference<Material>> for ReferenceModel<MaterialModel> {
    ...
    ) -> Result<Reference<Material>, SolveErrors> {
...
impl<'de> Solver<'de, VariantVariable> for VariantVariableModel {
    fn solve(
        self,
        storage_backend: &dyn resource::ReadStorageBackend,
    ) -> Result<VariantVariable, SolveErrors> {
        Ok(VariantVariable {
            name: self.name,
            r#type: self.r#type,
...
impl<'de> Solver<'de, Reference<Variant>> for ReferenceModel<VariantModel> {
    fn solve(
        self,
        storage_backend: &dyn resource::ReadStorageBackend,
    ) -> Result<Reference<Variant>, SolveErrors> {
        let (gr, reader) = storage_backend
            .read(self.id())
            .ok_or_else(|| SolveErrors::StorageError)?;
        let VariantModel {
...
impl<'de> Solver<'de, Reference<Shader>> for ReferenceModel<Shader> {
    fn solve(
        self,
        storage_backend: &dyn resource::ReadStorageBackend,
    ) -> Result<Reference<Shader>, SolveErrors> {
        let (gr, reader) = storage_backend
            .read(self.id())
            .ok_or_else(|| SolveErrors::StorageError)?;
        let Shader {
