...
pub fn generate_mip_chain<'a>(
    ...
    let bytes_per_pixel =
        bytes_per_pixel(format).ok_or(MipGenerationError::UnsupportedFormat(format))?;
    let expected_base_size = expected_size(width, height, bytes_per_pixel)
    ...
    loop {
        ...
        let next_size = expected_size(next_width, next_height, bytes_per_pixel)
            .ok_or(MipGenerationError::DimensionsTooLarge)?;
...
fn downsample_u8<const CHANNELS: usize>(
    source_width: u32,
    source_height: u32,
    source: &[u8],
    destination: &mut [u8],
...
fn downsample_u16<const CHANNELS: usize>(
    source_width: u32,
    source_height: u32,
    source: &[u8],
    destination: &mut [u8],
...
mod tests {
    ...
    fn generates_rgba8_chain_with_even_extent() {
        ...
        let generated =
            generate_mip_chain(Formats::RGBA8, width, height, &data).expect("mips must generate");
    ...
    fn generates_rgba16_chain() {
        ...
        let generated = generate_mip_chain(Formats::RGBA16, width, height, &data)
            .expect("16-bit mips must generate");
    ...
    fn rejects_invalid_buffer_size() {
        ...
        assert_eq!(
            error,
            MipGenerationError::BufferSizeMismatch {
                expected: 16,
                got: 3
            }
        );
    ...
    fn create_rgba16_pattern(width: u32, height: u32) -> Vec<u8> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                for channel in 0..4 {
                    let value = (((x as usize + 1) * 997)
                        + ((y as usize + 1) * 557)
                        + ((channel + 1) * 313)) as u16;
    ...
    fn scalar_mip_chain_u8<const CHANNELS: usize>(
        width: u32,
        height: u32,
        base_level: &[u8],
    ) -> Vec<ExpectedMipLevel> {
    ...
    fn scalar_mip_chain_u16<const CHANNELS: usize>(
        width: u32,
        height: u32,
        base_level: &[u8],
    ) -> Vec<ExpectedMipLevel> {
        ...
        loop {
            ...
            let next_height = (current_height / 2).max(1);
            let mut next_data =
                vec![0_u8; next_width as usize * next_height as usize * CHANNELS * 2];
            ...
            for y in 0..next_height as usize {
                ...
                for x in 0..next_width as usize {
                    ...
                    for channel in 0..CHANNELS {
                        let c = channel * 2;
                        let s00 =
                            u16::from_le_bytes([current_data[p00 + c], current_data[p00 + c + 1]])
                                as u32;
                        let s10 =
                            u16::from_le_bytes([current_data[p10 + c], current_data[p10 + c + 1]])
                                as u32;
                        let s01 =
                            u16::from_le_bytes([current_data[p01 + c], current_data[p01 + c + 1]])
                                as u32;
                        let s11 =
                            u16::from_le_bytes([current_data[p11 + c], current_data[p11 + c + 1]])
                                as u32;
                        let value = ((s00 + s10 + s01 + s11 + 2) / 4) as u16;
                        next_data[destination + c..destination + c + 2]
