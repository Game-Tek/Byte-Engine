...
The default `files` mode writes one hash-named payload file per resource. BELD records the selected mode in `resources.db`, and the application discovers it when opening the store. Clear the destination before changing its mode.
