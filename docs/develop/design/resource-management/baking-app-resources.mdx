...
BELD pauses new independent work when the budget is full, while active bakes and their dependencies continue until they release memory. One large asset can temporarily exceed the budget. See [Control bake memory](/docs/develop/design/resource-management/beld#control-bake-memory) for sizing guidance and the deadlock-safety reason for using a soft limit.
