...
Use BELD to turn your source assets into the complete resource store that a
release application loads.
...
Commit both the vendor directory and symlink, or recreate them in the same
bootstrap step in every developer and CI checkout. Install the matching BELD
release so its handlers and resource formats stay aligned with the engine:

```bash
cargo install beld --version 0.2.0 --locked
```

On Windows, the checkout must preserve directory symlinks. [Enable Developer Mode](https://learn.microsoft.com/en-us/windows/advanced-settings/developer-mode)
or otherwise grant link creation permission, and make sure Git doesn't
materialize the link as a plain text file. The current automated symlink
coverage runs on Unix hosts.
...
With no IDs, BELD recursively discovers all supported source assets, including linked engine shaders, standalone `.environment.bead` assets, images, meshes, materials, and audio. Unknown files and BEAD sidecars are skipped; `.environment.bead` files are standalone assets rather than sidecars. A standalone `.besl` file is discovered only when its adjacent `.besl.bead` exists; this keeps material-internal BESL sources out of the standalone shader pass. Explicitly requesting a `.besl` file without its sidecar fails the bake.
...
BELD pauses new independent work when the budget is full, while active bakes and their dependencies continue until they release memory. One large asset can temporarily exceed the budget. See [Control bake memory](/docs/develop/resource-management/beld#control-bake-memory) for sizing guidance and the deadlock-safety reason for using a soft limit.
...
```bash
beld --source "$PWD/assets" --destination "$PWD/resources" \
  --storage-mode packed bake
```

The default `files` mode writes one extensionless payload file per resource. Its basename contains the resource ID, decoded hash, and encoding. BELD records the selected mode and each resource's payload encoding in `resources.db`, and the application discovers them when opening the store. Clear the destination before changing its mode.
...
Run `beld bake` again after changing a source asset. BELD rebakes only the assets whose source files changed. After updating an engine asset processor, run `beld bake --force` instead, because processor changes don't change source file versions. Baking requested IDs updates their generated output but does not remove unrelated old resources. Use `beld clear` only when you intentionally want to discard the entire resource directory.
...
Next, [build and bundle your application](/docs/use/build) with the baked
`resources/` directory.
