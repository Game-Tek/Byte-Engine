...
Outputs can add a positive element count when a mesh stage emits per-primitive values:

```rust
out_instance_index: output<u32, 0, 126>;
```

Task and mesh stages share authored payload arrays. Task stages can also declare workgroup-local storage:
