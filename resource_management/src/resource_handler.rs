...
pub trait ResourceHandler {
	...
	fn create_resource<'a>(&'a self, info: &'a CreateInfo, resource_manager: &'a resource_manager::ResourceManager) -> utils::BoxedFuture<Result<Vec<ProcessedResources>, String>> {
		async move { Result::Ok(vec![]) }.boxed()
	}
}