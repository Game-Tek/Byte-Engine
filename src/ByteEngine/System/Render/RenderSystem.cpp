...
#include "ByteEngine/System/Resource/PipelineCacheResourceManager.h"
	...
	textures(16, GetPersistentAllocator()), apiAllocations(128, GetPersistentAllocator()), workloads(16, GetPersistentAllocator())
{
	...
	pipelinedFrames = static_cast<GTSL::uint8>(GTSL::Math::Clamp(static_cast<GTSL::uint32>(config[u8"buffer"].GetUint()), 2u, 3u));
