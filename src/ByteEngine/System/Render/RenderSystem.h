...
#include "ByteEngine/Render/RendererAllocator.h"
...
public:
	MAKE_HANDLE(GTSL::uint32, Buffer)
	MAKE_HANDLE(GTSL::uint32, Texture);
	MAKE_HANDLE(GTSL::uint32, RenderContext);
	
	explicit RenderSystem(const InitializeInfo& initializeInfo);
	~RenderSystem();
	[[nodiscard]] GTSL::uint8 GetCurrentFrame() const { return currentFrameIndex; }
	[[nodiscard]] GTSL::uint8 GetFrameIndex(GTSL::int32 frameDelta) const { return static_cast<GTSL::uint8>(frameDelta % pipelinedFrames); }
	GTSL::uint8 GetPipelinedFrames() const { return pipelinedFrames; }
	...
	MAKE_HANDLE(GTSL::uint32, CommandList);
	MAKE_HANDLE(GTSL::uint32, Workload);
	MAKE_HANDLE(GTSL::uint32, AccelerationStructure);
	MAKE_HANDLE(GTSL::uint32, BLASInstance);
	...
	WorkloadHandle CreateWorkload(GTSL::StringView name, GAL::QueueType type, GAL::PipelineStage pipeline_stages) {
		GTSL::uint32 index = workloads.Emplace();
	...
	CommandListHandle CreateCommandList(const GTSL::StringView name, GAL::QueueType type, GAL::PipelineStage pipeline_stages, bool isSingleFrame = true) {
		GTSL::uint32 index = commandLists.GetLength();
	...
	void StartCommandList(const CommandListHandle command_list_handle) {
		...
		if (auto& textureCopyData = commandListData.textureCopyDatas; textureCopyData) {
			...
			for (GTSL::uint32 i = 0; i < textureCopyData.GetLength(); ++i) {
	...
	void Submit(const GAL::QueueType queue_type, const GTSL::Range<const WorkUnit*> work_units, const WorkloadHandle workload_handle) {
		...
		for (GTSL::uint32 wui = 0; wui < work_units.ElementCount(); ++wui) {
	...
	void Present(WindowSystem* window_system, GTSL::Range<const RenderContextHandle*> render_context_handles, const GTSL::Range<const WorkloadHandle*> wait_workload_handles) {
		...
		GTSL::StaticVector<GTSL::uint32, 8> imageIndices;
	...
	void AllocateScratchBufferMemory(GTSL::uint32 size, GAL::BufferUse flags, GPUBuffer* buffer, RenderAllocation* allocation) {		
		GAL::MemoryRequirements memoryRequirements;
	...
	void AllocateLocalBufferMemory(GTSL::uint32 size, GAL::BufferUse flags, GPUBuffer* buffer, RenderAllocation* allocation) {
		GAL::MemoryRequirements memoryRequirements;
		buffer->GetMemoryRequirements(GetRenderDevice(), size, flags, &memoryRequirements);
	...
	RenderContextHandle CreateRenderContext(WindowSystem* window_system, WindowSystem::WindowHandle window_handle) {
		GTSL::uint32 renderContextIndex = renderx.GetLength();
	...
	void AddBufferUpdate(CommandListHandle command_list_handle, const BufferHandle source_buffer_handle, const BufferHandle destination_buffer_handle, GTSL::uint32 source_offset = 0, GTSL::uint32 destination_offset = 0) {
	...
	GTSL::uint32 GetBufferSize(const BufferHandle buffer_handle) const {
	...
	GTSL::uint32 GetShaderGroupHandleSize() const { return shaderGroupHandleSize; }
	GTSL::uint32 GetShaderGroupBaseAlignment() const { return shaderGroupBaseAlignment; }
	GTSL::uint32 GetShaderGroupHandleAlignment() const { return shaderGroupHandleAlignment; }
	...
	GTSL::uint32 GetBufferSubDataAlignment() const { return renderDevice.GetStorageBufferBindingOffsetAlignment(); }
	...
	GTSL::Range<byte*> GetTextureRange(TextureHandle textureHandle) {
		...
		GTSL::uint32 size = texture.Extent.Width * texture.Extent.Depth * texture.Extent.Height;
		size *= texture.format.GetSize();
	...
	BufferHandle CreateBuffer(GTSL::uint32 size, GAL::BufferUse flags, bool willWriteFromHost, const BufferHandle buffer_handle);

	AccelerationStructureHandle CreateTopLevelAccelerationStructure(GTSL::uint32 estimatedMaxInstances) {
		GTSL::uint32 tlasi = accelerationStructures.Emplace(true);
	...
	AccelerationStructureHandle CreateBottomLevelAccelerationStructure(GTSL::uint32 vertexCount, GTSL::uint32 vertexSize, GTSL::uint32 indexCount, GAL::IndexType indexType,  BufferHandle vertex_buffer_handle, BufferHandle index_buffer_handle, GTSL::uint32 vertex_buffer_byte_offset = 0, GTSL::uint32 index_buffer_byte_offset = 0, bool willUpdate = false, bool willRebuild = false, bool isOpaque = true) {
		GTSL::uint32 blasi = accelerationStructures.Emplace(false);
	...
	GTSL::uint32 CreateAABB(const GTSL::Matrix4& position, const GTSL::Vector3 size) {
	...
	BLASInstanceHandle AddBLASToTLAS(const AccelerationStructureHandle tlash, const AccelerationStructureHandle blash, GTSL::uint32 instance_custom_index, BLASInstanceHandle instance_handle) {
		auto& tlas = accelerationStructures[tlash()].TopLevel;

		GTSL::uint32 instanceIndex = 0;
...
#define BE_LOG_IF(cond, text) if(cond) { BE_LOG_WARNING(text); return; }
	...
	void SetAccelerationStructureInstanceIndex(AccelerationStructureHandle topLevel, BLASInstanceHandle instance_handle, GTSL::uint32 custom_index) {
	...
	void SetInstanceBindingTableRecordOffset(AccelerationStructureHandle topLevel, BLASInstanceHandle instance_handle, const GTSL::uint32 offset) {
...
private:
	...
	struct BufferCopyData {
		BufferHandle SourceBufferHandle, DestinationBufferHandle;
		GTSL::uint32 SourceOffset = 0, DestinationOffset = 0;
	};
	...
	struct BufferData {
		GTSL::uint32 Size = 0, Counter = 0;
		GAL::BufferUse Flags;
		GTSL::uint32 references = 0;
		GPUBuffer Buffer;
		RenderAllocation Allocation;
	...
	struct AccelerationStructureData {
		...
		struct TopLevelAccelerationStructure {
			...
			GTSL::StaticVector<GTSL::uint32, 8> freeSlots;
		};
		...
		struct BottomLevelAccelerationStructure {
			...
			GTSL::uint32 VertexByteOffset, IndexBufferByteOffset;
		};
	...
	GTSL::uint8 currentFrameIndex = 0;
	...
	void* allocateApiMemory(void* data, GTSL::uint64 size, GTSL::uint64 alignment);
	void* reallocateApiMemory(void* data, void* allocation, GTSL::uint64 size, GTSL::uint64 alignment);
	...
	GTSL::HashMap<GTSL::uint64, GTSL::Pair<GTSL::uint64, GTSL::uint64>, BE::PersistentAllocatorReference> apiAllocations;
	...
	GTSL::uint32 shaderGroupHandleAlignment = 0, shaderGroupBaseAlignment = 0, shaderGroupHandleSize = 0;
	GTSL::uint32 scratchBufferOffsetAlignment = 0;
