...
static Id assimpStringToId(const aiString& aiString) {
	return Id(GTSL::Range<const char8_t*>(aiString.length + 1, aiString.length + 1, reinterpret_cast<const char8_t*>(aiString.data)));
}
...
AnimationResourceManager::AnimationResourceManager(const InitializeInfo& initialize_info): ResourceManager(initialize_info, u8"AnimationResourceManager"), animations(32, 0.3f, GetPersistentAllocator())
{
	...
	case GTSL::File::OpenResult::CREATED: {
		...
		while (auto query = fileQuery()) {
			...
			loadSkeleton(static_cast<GTSL::Range<const GTSL::uint8*>>(buffer), skeletonData, skeletonDataBuffer);
			loadAnimation(static_cast<GTSL::Range<const GTSL::uint8*>>(buffer), animationData, animationDataBuffer);
...
void AnimationResourceManager::loadSkeleton(const GTSL::Range<const GTSL::uint8*> sourceBuffer, SkeletonData& skeletonData,
                                            GTSL::Buffer<BE::TAR>& meshDataBuffer)
{
	...
	for (GTSL::uint32 b = 0; b < mesh->mNumBones; ++b) {
		...
		for (GTSL::uint32 w = 0; w < assimpBone->mNumWeights; ++w) {
...
void AnimationResourceManager::loadAnimation(const GTSL::Range<const GTSL::uint8*> sourceBuffer, AnimationData& animationData,
	GTSL::Buffer<BE::TAR>& meshDataBuffer)
{
	Assimp::Importer importer;
	const auto* const scene = importer.ReadFileFromMemory(sourceBuffer.begin(), sourceBuffer.Bytes(),
	...
	for (GTSL::uint32 a = 0; a < scene->mNumAnimations; ++a)
	{
		...
		BE_ASSERT(assimpAnimation->mDuration - ((GTSL::float64)(GTSL::uint64)assimpAnimation->mDuration) != 0.0,
			"Animation is not round number");
		animation.FrameCount = static_cast<GTSL::uint64>(assimpAnimation->mDuration);

		animation.FPS = static_cast<GTSL::float32>(assimpAnimation->mTicksPerSecond == 0.0
			                                           ? 30.0
			                                           : assimpAnimation->mTicksPerSecond);

		for (GTSL::uint32 c = 0; c < assimpAnimation->mNumChannels; ++c) {
			BE_ASSERT(assimpStringToId(assimpAnimation->mChannels[c]->mNodeName) == boneNames[c], "Order doesn't match");
		}

		for (GTSL::uint32 frameIndex = 0; frameIndex < animation.FrameCount; ++frameIndex)
		{
			...
			for (GTSL::uint32 b = 0; b < assimpAnimation->mNumChannels; ++b)
			{
