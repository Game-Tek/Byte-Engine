...
public:
	...
	{
		GTSL::Matrix4 Offset;
		GTSL::Vector<GTSL::Pair<GTSL::uint32, GTSL::float32>, BE::PAR> AffectedVertices;
	...
	struct SkeletonData : Data
	{
		...
		GTSL::HashMap<GTSL::StringView, GTSL::uint32, BE::PAR> BonesMap;
	...
	{
		GTSL::uint32 FrameCount = 0, FPS = 0;
...
private:
	void loadSkeleton(const GTSL::Range<const GTSL::uint8*> sourceBuffer, SkeletonData& skeletonData, GTSL::Buffer<BE::TAR>& meshDataBuffer);
	void loadAnimation(const GTSL::Range<const GTSL::uint8*> sourceBuffer, AnimationData& animationData, GTSL::Buffer<BE::TAR>& meshDataBuffer);
