	...
	if(indexFile.Read(file_buffer))
	{
		...
		while(auto queryResult = file_query()) {
			...
			if (!audioResourceInfos.Find(GTSL::StringView(fileName))) {
				...
				GTSL::uint8 riff[4];                      // RIFF string
				GTSL::uint32 overall_size = 0;               // overall size of file in bytes
				GTSL::uint8 wave[4];                      // WAVE string
				GTSL::uint8 fmt_chunk_marker[4];          // fmt string with trailing null char
				GTSL::uint32 length_of_fmt = 0;                 // length of the format data
				GTSL::uint16 format_type = 0;                   // format type. 1-PCM, 3- IEEE float, 6 - 8bit A law, 7 - 8bit mu law
				GTSL::uint16 channels = 0;                      // no.of channels
				GTSL::uint32 sample_rate = 0;                   // sampling rate (blocks per second)
				GTSL::uint32 byte_rate = 0;                      // SampleRate * NumChannels * BitsPerSample/8
				GTSL::uint16 block_align = 0;                   // NumChannels * BitsPerSample/8
				GTSL::uint16 bits_per_sample = 0;               // bits per sample, 8- 8bits, 16- 16 bits etc
				GTSL::uint8 data_chunk_header[4];        // DATA string or FLLR string
				GTSL::uint32 data_size = 0;                     // NumSamples * NumChannels * BitsPerSample/8 - size of the next chunk that will be read
				...
				data.ChannelCount = static_cast<GTSL::uint8>(channels);
				data.SampleRate = sample_rate;
				data.BitDepth = static_cast<GTSL::uint8>(bits_per_sample);
				...
				data.ByteOffset = (GTSL::uint32)packageFile.GetSize();

				packageFile.Write(GTSL::Range<const GTSL::uint8*>(data_size, wavBuffer.GetData() + wavBuffer.GetReadPosition()));
