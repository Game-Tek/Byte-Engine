...
public:
	...
	ResourceManager(const InitializeInfo& info, const char8_t* name) : System(info,name) {}

	static GTSL::StaticString<512> GetUserResourcePath(const GTSL::Range<const char8_t*>& fileWithExtension);
	static GTSL::StaticString<512> GetUserResourcePath(const GTSL::Range<const char8_t*>& fileName, const GTSL::Range<const char8_t*>& extensions);

	GTSL::StaticString<512> GetResourcePath(const GTSL::Range<const char8_t*>& fileWithExtension);
	GTSL::StaticString<512> GetResourcePath(const GTSL::Range<const char8_t*>& fileName, const GTSL::Range<const char8_t*>& extensions);
...
#define EXTRACT_BODY Extract(extractInfo.ByteOffset, buffer);
	...
	static constexpr GTSL::uint8 MAX_THREADS = 32;
protected:
	void InitializePackageFiles(GTSL::StaticVector<GTSL::File, MAX_THREADS>& filesPerThread, GTSL::Range<const char8_t*> path);
};

struct EntryHeader
{
	GTSL::uint64 DataOffset = 0;
	GTSL::uint64 DataSize = 0;
...
struct ResourceFiles
{
	void Start(const GTSL::StringView string)
	{
		...
		if (!(a && b && c) or !table.GetSize() or !index.GetSize() or !data.GetSize()) 
		{
			...
			table.Write(5, reinterpret_cast<const GTSL::uint8*>("BETBL"));
			index.Write(5, reinterpret_cast<const GTSL::uint8*>("BEIDX"));
			data.Write(5, reinterpret_cast<const GTSL::uint8*>("BEPKG"));
		}
		else {
			...
			while (true)
			{
				...
				if (!read) { break; }
				for (GTSL::uint32 i = 0; i < buffer.GetLength() / sizeof(TableEntry); ++i) {
					TableEntry table_entry;
					buffer.Read(sizeof(TableEntry), reinterpret_cast<GTSL::uint8*>(&table_entry));
	...
	template<typename T>
	bool AddEntry(const GTSL::StringView name, T* indexDataPointer, GTSL::Range<const GTSL::uint8*> dataPointer)
	{
		...
		table.Write(sizeof(TableEntry), reinterpret_cast<const GTSL::uint8*>(&table_entry));
		...
		index.Write(GTSL::Math::RoundUpByPowerOf2(sizeof(T), BLOCK_SIZE), reinterpret_cast<const GTSL::uint8*>(indexDataPointer));
	...
	{
		if (!tableMap.Find(static_cast<GTSL::uint64>(Id(name)))) { return false; }

		auto indexDataOffset = tableMap.At(static_cast<GTSL::uint64>(Id(name)));
	...
	{
		return tableMap.Find(static_cast<GTSL::uint64>(name));
	}
	...
	bool LoadData(auto& info, GTSL::Range<GTSL::uint8*> buffer)
	{
		data.SetPointer(info.Header.DataOffset);
	...
	bool LoadData(auto& info, GTSL::Range<GTSL::uint8*> buffer, GTSL::uint32 offset, GTSL::uint32 size) {
...
private:
	static constexpr GTSL::uint64 BLOCK_SIZE = 1024u;
	...
	GTSL::HashMap<GTSL::uint64, GTSL::uint64, GTSL::DefaultAllocatorReference> tableMap;
};
