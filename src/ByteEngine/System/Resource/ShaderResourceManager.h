...
#include "ByteEngine/Graphics/ShaderCompilation.hpp"
#include "ByteEngine/Game/ApplicationManager.h"
#include "ByteEngine/Render/ShaderGenerator.h"
#include "ByteEngine/Graph.hpp"

#include "ByteEngine/Resources/PermutationManager.hpp"
#include "ByteEngine/Resources/CommonPermutation.hpp"
#include "ByteEngine/Resources/ForwardPermutation.hpp"
#include "ByteEngine/Resources/UIPermutation.hpp"
#include "ByteEngine/Resources/RayTracePermutation.hpp"
...
auto operator<<(auto& buffer, const GTSL::Vector<T, A>& vector) -> decltype(buffer)& {
	...
	for (GTSL::uint32 i = 0; i < vector.GetLength(); ++i) { buffer << vector[i]; }
	return buffer;
}
...
auto operator>>(auto& buffer, GTSL::Vector<T, A>& vector) -> decltype(buffer)& {
	GTSL::uint32 length;
	buffer >> length;
	for (GTSL::uint32 i = 0; i < length; ++i) { buffer >> vector.EmplaceBack(); }
	return buffer;
}
...
auto Read(auto& buffer, GTSL::Vector<T, A>& vector, const BE::PAR& allocator) -> decltype(buffer)& {
	GTSL::uint32 length;
	buffer >> length;
	for (GTSL::uint32 i = 0; i < length; ++i) { Extract(vector.EmplaceBack(), buffer); }
	return buffer;
}

template<GTSL::uint8 S>
auto operator>>(auto& buffer, GTSL::ShortString<S>& string) -> decltype(buffer)& {
	...
	for (GTSL::uint32 i = 0; i < length; ++i) { buffer >> const_cast<char8_t*>(string.begin())[i]; }
	return buffer;
}
...
static unsigned long long quickhash64(const GTSL::Range<const GTSL::uint8*> range) { // set 'mix' to some value other than zero if you want a tagged hash          
...
public:
	...
	struct RayGenShader { GTSL::uint8 Recursion = 1; };
	...
	struct ShaderInfo {
		...
		GAL::ShaderType Type; GTSL::uint64 Hash = 0;
	...
	struct ShaderGroupData : Data {
		...
		GTSL::Vector<GTSL::uint64, BE::PAR> Shaders;
	...
	struct ShaderGroupInfo {
		...
		struct RayTraceData {
			...
			struct Group {
				GTSL::StaticVector<GTSL::uint32, 8> ShadersPerGroup;
			} Groups[4];
		} RayTrace;
	};
	...
	template<typename... ARGS>
	void LoadShaderGroup(ApplicationManager* gameInstance, ShaderGroupInfo shader_group_info, TaskHandle<ShaderGroupInfo, GTSL::Range<GTSL::uint8*>, ARGS...> dynamicTaskHandle, GTSL::Range<GTSL::uint8*> buffer, ARGS&&... args) {
...
private:
	...
	GTSL::HashMap<Id, GTSL::uint64, BE::PersistentAllocatorReference> shaderGroupInfoOffsets;
	GTSL::HashMap<GTSL::uint64, GTSL::uint64, BE::PersistentAllocatorReference> shaderInfoOffsets, shaderOffsets, shaderInfoPointers, shadersPointer;
	...
	GTSL::Buffer<BE::TransientAllocatorReference> compileShader(const GTSL::JSON<BE::TAR>& json,
	                                                            ...
	                                                            scopes);
	void serializeShader(const GTSL::JSON<BE::TAR>& json, GTSL::Range<const GTSL::uint8*> shader_binary_buffer);
	...
	void loadShaderGroup(TaskInfo taskInfo, Id shaderGroupName, TaskHandle<ShaderGroupInfo, ARGS...> dynamicTaskHandle, ARGS... args) { //TODO: check why can't use ARGS&&
		...
		GTSL::uint32 shaderCount;
		shaderGroupInfosFile >> shaderCount;

		for (GTSL::uint32 s = 0; s < shaderCount; ++s) {
			GTSL::uint64 shaderHash;
...

			{
				...
				shader.Size = GTSL::uint64(shaderJSON[u8"binarySize"]);
				shader.Hash = GTSL::uint64(shaderJSON[u8"binaryHash"]);
		...
		{
			GTSL::uint32 parameterCount;
			shaderGroupInfosFile >> parameterCount;

			for (GTSL::uint32 p = 0; p < parameterCount; ++p) {
		...
		{
			GTSL::uint32 tagCount;
			shaderGroupInfosFile >> tagCount;

			for (GTSL::uint32 p = 0; p < tagCount; ++p) {
		...
		if (rayTrace) {
			...
			for (GTSL::uint32 i = 0; i < 4; ++i) {
				GTSL::uint32 groupCount; shaderGroupInfosFile >> groupCount;

				for (GTSL::uint32 j = 0; j < groupCount; ++j) {
	...
	template<typename... ARGS>
	void loadShaders(TaskInfo taskInfo, ShaderGroupInfo shader_group_info, GTSL::Range<GTSL::uint8*> buffer, TaskHandle<ShaderGroupInfo, GTSL::Range<GTSL::uint8*>, ARGS...> dynamicTaskHandle, ARGS... args) {
...
struct VertexPermutationManager {
	VertexPermutationManager(GPipeline* pipeline) {
		for(GTSL::uint32 i = 0; i < vertexPermutations; ++i) {

			GTSL::StaticVector<StructElement, 8> structElements;

			for(GTSL::uint32 j = 0; j < vertexPermutations[i]; ++j) {
			}
...
inline ShaderResourceManager::ShaderResourceManager(const InitializeInfo& initialize_info) : ResourceManager(initialize_info, u8"ShaderResourceManager"), shaderGroupInfoOffsets(8, GetPersistentAllocator()), shaderInfoOffsets(8, GetPersistentAllocator()), shaderOffsets(8, GetPersistentAllocator()), shaderInfoPointers(8, GetPersistentAllocator()), shadersPointer(8, GetPersistentAllocator()) {
	...
	struct IFile {
		...
		GTSL::uint64 FileHash = 0;
		GTSL::uint64 Pointer = 0ull;
		State state;
	};

	GTSL::HashMap<GTSL::uint64, Graph<IFile>, BE::TAR> dependencyTree(128, GetTransientAllocator());
...

	{
		...
		while(offset != shaderInfoTableFile.GetSize()) {
			offset = ReadIndexEntry(shaderInfoTableFile, offset, [&](const GTSL::uint64 offs, const GTSL::StringView name) {
...

	{
		...
		while(offset != shadersTableFile.GetSize()) {
			offset = ReadIndexEntry(shadersTableFile, offset, [&](const GTSL::uint64 offs, const GTSL::StringView name) {
	...
	auto printDependencyTree = [&](GTSL::uint64 parent_file_name_hash, const Graph<IFile>& node, auto&& self) -> void {
...

	{
		...
		while(offset != shaderGroupsTableFile.GetSize()) {
			offset = ReadIndexEntry(shaderGroupsTableFile, offset, [&](const GTSL::uint64 offset, const GTSL::StringView name) { shaderGroupInfoOffsets.Emplace(Id(name), offset); });
		}
	}
}
...
inline void ShaderResourceManager::processShaderGroup(const GTSL::JSON<BE::PAR>& json, GPipeline& pipeline,
	...
	pipeline.DeclareConstant(shaderGroupScope, { u8"GTSL::uint32", u8"DEBUG", u8"0" });
...
inline void ShaderResourceManager::makeShaderGroup(const GTSL::JSON<BE::PAR>& json, GPipeline& pipeline, PermutationManager* root_permutation, ShaderGroupDataSerialize* shader_group_data_serialize, const ShaderMap& shader_map) {
	...
	auto evaluateForPermutation = [&](PermutationManager* permutation_manager, GTSL::StaticVector<GPipeline::ElementHandle, 16> permutationScopes, auto&& self) -> void {
		...
		for (auto s : json[u8"shaders"]) {
			...
			GTSL::uint32 i = 0, j = 0; bool found = false;
			...
			for(; i < permutation_manager->JSON[u8"options"]; ++i) {
				if(permutation_manager->JSON[u8"options"][i][u8"domain"] == GTSL::StringView(shaderGroupDomain)) {
					for(; j < permutation_manager->JSON[u8"options"][i][u8"subsets"]; ++j) {
						...
						for(GTSL::uint32 k = 0; k < permutation_manager->JSON[u8"options"][i][u8"subsets"][j][u8"sourceClasses"]; ++k) {
			...
			for(GTSL::uint32 t = 0u; t < tokens; ++t) {
				...
				GTSL::uint32 startFunctionCode = t;

				GTSL::uint32 openCloseCounter = 1u; // Must be one because we skipped over open brace
				while(true) {
					if(tokens[t].Name == u8"}") {
						--openCloseCounter;
						if(!openCloseCounter) {
...

	{
		...
		if (shader_group_data_serialize.RayTrace) {
			...
			for (GTSL::uint32 i = 0; i < 4; ++i) {
				...
				for (GTSL::uint32 j = 0; j < rayTraceData.Groups[i].ShadersPerGroup.GetLength(); ++j) {
...
inline void ShaderResourceManager::serializeShader(const GTSL::JSON<BE::TAR>& json, GTSL::Range<const GTSL::uint8*> shader_binary_buffer) {
