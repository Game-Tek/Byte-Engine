...
public:
	...
	struct StaticMeshInfo : SData {
		struct SubMeshData : SubData {
			...
			*/
			DEFINE_MEMBER(GTSL::uint32, VertexCount)

			/**
			 * \brief Number of indeces the loaded mesh contains. Every face can only have three indeces.
			 */
			DEFINE_MEMBER(GTSL::uint32, IndexCount)
			DEFINE_MEMBER(GTSL::uint32, MaterialIndex)
			DEFINE_MEMBER(GTSL::Vector3, BoundingBox)
			DEFINE_MEMBER(GTSL::float32, BoundingRadius)
		 ...
		 */
		DEFINE_MEMBER(GTSL::uint8, IndexSize)
		DEFINE_MEMBER(GTSL::Vector3, BoundingBox)
		DEFINE_MEMBER(GTSL::float32, BoundingRadius)
		...
		GTSL::uint32 GetVertexSize() const
		{
			GTSL::uint32 size = 0;
	...
	template<typename... ARGS>
	void LoadStaticMesh(ApplicationManager* gameInstance, StaticMeshInfo staticMeshInfo, GTSL::uint32 verticesOffset, GTSL::Range<byte*> vertexBuffer, uint32 indicesOffset, GTSL::Range<byte*> indexBuffer, TaskHandle<StaticMeshInfo, ARGS...> dynamicTaskHandle, ARGS&&... args) {
...
private:
	...
	template<typename... ARGS>
	void loadMesh(TaskInfo taskInfo, StaticMeshInfo staticMeshInfo, GTSL::uint32 verticesOffset, GTSL::Range<byte*> vertexBuffer, uint32 indicesOffset, GTSL::Range<byte*> indexBuffer, TaskHandle<StaticMeshInfo, ARGS...> dynamicTaskHandle, ARGS... args)
	{
