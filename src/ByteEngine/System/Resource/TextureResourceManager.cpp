...
TextureResourceManager::TextureResourceManager(const InitializeInfo& initialize_info) : ResourceManager(initialize_info, u8"TextureResourceManager") {
	...
	while (auto queryResult = file_query()) {
		...
		if (!resource_files_.Exists(hashed_name)) {
			...
			GTSL::int32 x = 0, y = 0, channel_count = 0;
			stbi_info_from_memory(textureFileBuffer.GetData(), textureFileBuffer.GetLength(), &x, &y, &channel_count);
			auto finalChannelCount = GTSL::NextPowerOfTwo(static_cast<GTSL::uint32>(channel_count));
			GTSL::uint8* data = nullptr;

			{
				...
				for(auto& e : substrings) {
					...
					break; case GTSL::Hash(u8"hdr"): {
						data = reinterpret_cast<GTSL::uint8*>(stbi_loadf_from_memory(textureFileBuffer.GetData(), textureFileBuffer.GetLength(), &x, &y, &channel_count, finalChannelCount));
						colorSpace = GAL::ColorSpaces::LINEAR;
					}
					}
				}
				...
				if(FindString(queryResult.Get(), u8"GLOSS")) { // If texture is a gloss texture, convert to roughness
					for(GTSL::uint32 i = 0; i < x * y * finalChannelCount; ++i) {
						data[i] = static_cast<GTSL::uint8>(GTSL::Math::InvertRange(static_cast<GTSL::uint32>(data[i]), 255u));
					}
				}
			}
			...
			const GTSL::uint32 size = static_cast<GTSL::uint32>(x) * y * finalChannelCount;
			texture_info.Extent = { static_cast<GTSL::uint16>(x), static_cast<GTSL::uint16>(y), 1 };
			...
			resource_files_.AddEntry(fileName, &texture_info, { size, static_cast<const GTSL::uint8*>(data) });
