...
namespace GAL
{
	...
	void setName(T* handle, const GTSL::StringView name) {
		if constexpr (_DEBUG) {
			if (name.GetBytes() != 0)
				handle->SetPrivateData(WKPDID_D3DDebugObjectName, name.GetBytes(), name.GetData());
		}
	}
	...
	inline DXGI_FORMAT ToDX12(const Format format) {
		...
		case Format::RG_I32: return DXGI_FORMAT_R32G32_UINT;
		default: return DXGI_FORMAT_UNKNOWN;
		}
	}
	...
	inline D3D12_SHADER_VISIBILITY ToDX12(const ShaderStage shaderStage) {
		...
		//TranslateMask(ShaderStages::VERTEX, D3D12_SHADER_VISIBILITY_VERTEX, shaderStage, shaderVisibility);
		//TranslateMask(ShaderStages::FRAGMENT, D3D12_SHADER_VISIBILITY_PIXEL, shaderStage, shaderVisibility);
		//TranslateMask(ShaderStages::COMPUTE, D3D12_SHADER_VISIBILITY_DOMAIN, shaderStage, shaderVisibility);
