...
pub struct VisibilityShaderGenerator {
	...
	lighting_data_struct: jspd::parser::NodeReference,
...
impl VisibilityShaderGenerator {
	pub fn new(scope: NodeReference) -> Self {
		...
		let light_struct = NodeReference::r#struct("Light", vec![NodeReference::member("view_matrix", "mat4f"), NodeReference::member("projection_matrix", "mat4f"), NodeReference::member("vp_matrix", "mat4f"), NodeReference::member("position", "vec3f32"), NodeReference::member("color", "vec3f32"), NodeReference::member("light_type", "u8")]);
		let material_struct = NodeReference::r#struct("Material", vec![NodeReference::member("textures", "mat4f")]);
		let lighting_data_struct = NodeReference::r#struct("LightingData", vec![NodeReference::member("light_count", "u32"), NodeReference::member("lights", "Light[16]")]);

		let set0_binding1 = NodeReference::binding("mesh_buffer", NodeReference::buffer("MeshBuffer", vec![NodeReference::member("meshes", "Mesh[64]")]), 0, 1, true, false);
		let set0_binding2 = NodeReference::binding("positions", NodeReference::buffer("Positions", vec![NodeReference::member("positions", "vec3f32[8192]")]), 0, 2, true, false);
		let set0_binding3 = NodeReference::binding("normals", NodeReference::buffer("Normals", vec![NodeReference::member("normals", "vec3f32[8192]")]), 0, 3, true, false);
		...
		let set1_binding0 = NodeReference::binding("material_count", NodeReference::buffer("MaterialCount", vec![NodeReference::member("material_count", "u32[1920 * 1080]")]), 1, 0, true, false);
		let set1_binding1 = NodeReference::binding("material_offset", NodeReference::buffer("MaterialOffset", vec![NodeReference::member("material_offset", "u32[1920 * 1080")]), 1, 1, true, false);
		let set1_binding4 = NodeReference::binding("pixel_mapping", NodeReference::buffer("PixelMapping", vec![NodeReference::member("pixel_mapping", "vec2u16[1920 * 1080]")]), 1, 4, true, false);
		...
		let set2_binding4 = NodeReference::binding("lighting_data", NodeReference::buffer("LightingBuffer", vec![NodeReference::member("lighting_data", "LightingData")]), 2, 4, true, false);
		let set2_binding5 = NodeReference::binding("material_buffer", NodeReference::buffer("MaterialBuffer", vec![NodeReference::member("materials", "Material")]), 2, 5, true, false);
		...
		let geometry_smith = NodeReference::function("geometry_smith", vec![NodeReference::member("n", "vec3f32"), NodeReference::member("v", "vec3f32"), NodeReference::member("l", "vec3f32"), NodeReference::member("roughness", "f32")], "f32", vec![NodeReference::glsl("return geometry_schlick_ggx(max(dot(n, v), 0.0), roughness) * geometry_schlick_ggx(max(dot(n, l), 0.0), roughness);", Vec::new(), Vec::new())]);
		let fresnel_schlick = NodeReference::function("fresnel_schlick", vec![NodeReference::member("cos_theta", "f32"), NodeReference::member("f0", "vec3f32")], "vec3f32", vec![NodeReference::glsl("return f0 + (1.0 - f0) * pow(clamp(1.0 - cos_theta, 0.0, 1.0), 5.0);", Vec::new(), Vec::new())]);
		
		let barycentric_deriv = NodeReference::r#struct("BarycentricDeriv", vec![NodeReference::member("lambda", "vec3f32"), NodeReference::member("ddx", "vec3f32"), NodeReference::member("ddy", "vec3f32")]);

		let calculate_full_bary = NodeReference::function("calculate_full_bary", vec![], "BarycentricDeriv", vec![NodeReference::glsl("BarycentricDeriv ret = BarycentricDeriv(vec3(0), vec3(0), vec3(0)); vec3 invW = 1.0 / vec3(pt0.w, pt1.w, pt2.w); vec2 ndc0 = pt0.xy * invW.x; vec2 ndc1 = pt1.xy * invW.y; vec2 ndc2 = pt2.xy * invW.z; float invDet = 1.0 / determinant(mat2(ndc2 - ndc1, ndc0 - ndc1)); ret.ddx = vec3(ndc1.y - ndc2.y, ndc2.y - ndc0.y, ndc0.y - ndc1.y) * invDet * invW; ret.ddy = vec3(ndc2.x - ndc1.x, ndc0.x - ndc2.x, ndc1.x - ndc0.x) * invDet * invW; float ddxSum = dot(ret.ddx, vec3(1)); float ddySum = dot(ret.ddy, vec3(1)); vec2 deltaVec = pixelNdc - ndc0; float interpInvW = invW.x + deltaVec.x * ddxSum + deltaVec.y * ddySum; float interpW = 1.0 / interpInvW; ret.lambda.x = interpW * (invW.x + deltaVec.x * ret.ddx.x + deltaVec.y * ret.ddy.x); ret.lambda.y = interpW * (0.0    + deltaVec.x * ret.ddx.y + deltaVec.y * ret.ddy.y); ret.lambda.z = interpW * (0.0    + deltaVec.x * ret.ddx.z + deltaVec.y * ret.ddy.z); ret.ddx *= (2.0 / winSize.x); ret.ddy *= (2.0 / winSize.y); ddxSum  *= (2.0 / winSize.x); ddySum  *= (2.0 / winSize.y);  float interpW_ddx = 1.0 / (interpInvW + ddxSum); float interpW_ddy = 1.0 / (interpInvW + ddySum);  ret.ddx = interpW_ddx * (ret.lambda * interpInvW + ret.ddx) - ret.lambda; ret.ddy = interpW_ddy * (ret.lambda * interpInvW + ret.ddy) - ret.lambda; return ret;", Vec::new(), Vec::new())]);
	...
	fn transform(&self, program_state: &mut jspd::parser::ProgramState,) {
		...
		match m.node_mut() {
			jspd::parser::Nodes::Function { statements, .. } => {
				statements.insert(0, jspd::parser::NodeReference::glsl(a,/* vec![push_constant, material_offset, meshes, meshlets, materials, primitive_indices, vertex_indices, positions, normals]*/ Vec::new(), Vec::new()));
				statements.push(jspd::parser::NodeReference::glsl(b,/*vec![lighting_data, out_albedo, out_diffuse]*/ Vec::new(), Vec::new()));
			}
			_ => {}
		}
