...
pub trait ResourceHandler {
	...
	fn process(&self, resource_manager: &resource_manager::ResourceManager, asset_url: &str) -> Pin<Box<dyn std::future::Future<Output = Result<Vec<ProcessedResources>, String>>>>;

	fn get_deserializers(&self) -> Vec<(&'static str, Box<dyn Fn(&polodb_core::bson::Document) -> Box<dyn Resource> + Send>)>;

	fn read(&self, _resource: &Box<dyn Resource>, file: &mut std::fs::File, buffers: &mut [Stream]) -> Pin<Box<dyn std::future::Future<Output = ()>>>;
}