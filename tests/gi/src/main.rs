...
fn main() {
	...
	let _floor: EntityHandle<Mesh> = core::spawn_as_child(space_handle.clone(), Mesh::new("Box", "WhiteSolid", maths_rs::Mat4f::from_translation(Vector3::new(0.0, -0.5, 1.0)) * maths_rs::Mat4f::from_scale(Vector3::new(5.0, 1.0, 5.0))));
	let _wall: EntityHandle<Mesh> = core::spawn_as_child(space_handle.clone(), Mesh::new("Box", "WhiteSolid", maths_rs::Mat4f::from_translation(Vector3::new(0.0, -1.0, 1.0)) * maths_rs::Mat4f::from_scale(Vector3::new(5.0, 10.0, 1.0))));
	let _a: EntityHandle<Mesh> = core::spawn_as_child(space_handle.clone(), Mesh::new("Suzanne", "WhiteSolid", maths_rs::Mat4f::from_translation(Vector3::new(0.0, 0.5, 0.0)) * maths_rs::Mat4f::from_scale(Vector3::new(0.4, 0.4, 0.4))));
