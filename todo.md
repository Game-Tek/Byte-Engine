...
- Skip the frame instead of panicking when a swapchain image cannot be acquired. Metal `nextDrawable` returns `None` when the pool is exhausted or the layer is hidden, and Vulkan currently substitutes image index 0 on an acquire error. Return `Option<SwapchainAcquisition>` from both acquisition paths; the renderer already keeps a per-window `None` slot for unusable extents, so a failed acquisition takes the same path and the window is skipped for that frame.
