...
- Close the BESL language gaps that SSGI had to work around, then remove the workarounds from `crates/byte-engine/assets/rendering/visibility/ssgi-*.besl`. BESL has no `else`; `min`, `max`, and `clamp` take no integer operands; `length` and `mix` lack `vec2f` overloads and `round` lacks an `f32` overload; assigning one component of a local vector fails with "Accessor is not rooted in a buffer binding"; and a local named `x` or `y` collides with vector member access. Each gap needs the parser or lexer, the VM, and the MSL, HLSL, and GLSL backends.
