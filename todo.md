...
- Close the BESL language gaps that SSGI had to work around, then remove the workarounds from `crates/byte-engine/assets/rendering/visibility/ssgi-*.besl`. BESL has no `else`; assigning one component of a local vector fails with "Accessor is not rooted in a buffer binding"; and a local named `x` or `y` collides with vector member access. More generally, a local named like a struct field breaks access to that field: `let model: Transform = …; model.model[0]` lowers to `.0` in MSL. And after a store through a binding named like its own member, such as `views.views[i].view_projection = …`, later statements resolve the binding to that store's access instead of the binding, so the MSL loses its `resources.` root and the VM rejects the store as "Accessor is not rooted in a buffer binding". `directional-shadow-cascade-fit.besl` works around both by naming its locals apart from `View`'s fields and its binding `cascade_views`. Each gap needs the parser or lexer, the VM, and the MSL, HLSL, and GLSL backends.
